...
class PullPipeline:
    def __init__(
        ...
        self._trash_root = trash_root or Path(
            "~/.local/share/corpus-forge/trash"
        ).expanduser()
    ...
    def start(self, source_root: Path, poll_interval_s: float = 5.0) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run_loop, args=(poll_interval_s,), daemon=True
        )
        self._thread.start()
    ...
    def _handle_conflict(
        self, revision: dict, path: Path, local_hash: str | None
    ) -> None:
