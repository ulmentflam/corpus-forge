...
def _render_skeleton_join_config(
    dsn: str, *, embed_drain: bool = False, ingest_watch: bool = True
) -> str:
    """Render the minimal local skeleton the shared scope merges onto.

    Backend postgres + the join DSN, an empty ``[daemon]`` block (daemon
    defaults), a ``[service]`` block (RFC fleet-5), and a root-scope
    ``datasets = []`` (no local datasets yet; shared datasets land as
    commented blocks the operator fills in).  The ``datasets = []`` bare
    key MUST precede any ``[section]`` header — TOML binds bare keys to the
    most-recent table.

    ``embed_drain`` / ``ingest_watch`` seed the joined host's ``[service]``
    block: a joined host drains the shared embed backlog via the managed
    service (``embed_drain = true``), and a pure-drain GPU box turns the
    ingest watcher off (``ingest_watch = false``).  Defaults here are the
    *local* defaults (off / on) so the skeleton is safe even if a caller
    omits them; :func:`run_join` passes the join-resolved values.
    """
    out: list[str] = [
        ...
        "[daemon]",
        "",
        # RFC fleet-5 — the managed service drains the shared embed backlog.
        # `embed_drain` turns on the daemon's claim-based drain loop;
        # `ingest_watch` stays on unless this is a pure-drain GPU box.
        "[service]",
        f"embed_drain = {'true' if embed_drain else 'false'}",
        f"ingest_watch = {'true' if ingest_watch else 'false'}",
        "",
    ]
    return "\n".join(out) + "\n"


def _join_default_ingest_watch() -> bool:
    """Default ``ingest_watch`` for a joined host: OFF on a capable GPU box
    (pure-drain), ON otherwise.

    Mirrors :func:`_suggest_lanes_from_accelerator`'s big-hardware probe
    (``recommend_embedder_preset(...).n_gpu_layers != 0``).  Degrades to
    ``True`` (watcher on) if the probe raises — never leave a joined host
    unable to ingest because a probe failed.
    """
    try:
        info = detect_accelerator()
        preset = recommend_embedder_preset(info)
...
def render_join_config(
    dsn: str,
    shared_body: Mapping[str, Any] | None,
    *,
    ...
    skeleton = _render_skeleton_join_config(dsn, embed_drain=embed_drain, ingest_watch=ingest_watch)
...
def run_join(
    ...
    *,
    ...
    interactive: bool = True,
    stream_out: IO[str] | None = None,
    embed_drain: bool | None = None,
    ingest_watch: bool | None = None,
) -> tuple[Path, list[str]]:
    ...
    # RFC fleet-5 — resolve the joined host's [service] toggles. A joined
    # host drains the shared backlog by default (embed_drain=true); an
    # explicit --embed-drain/--no-embed-drain wins. ingest_watch defaults
    # OFF on a capable GPU box (pure-drain), ON otherwise, unless an
    # explicit --ingest-watch/--no-ingest-watch was passed.
    resolved_embed_drain = True if embed_drain is None else embed_drain
    resolved_ingest_watch = _join_default_ingest_watch() if ingest_watch is None else ingest_watch

    config_text, awaiting = render_join_config(
        dsn,
        shared_body,
        embed_drain=resolved_embed_drain,
        ingest_watch=resolved_ingest_watch,
    )
