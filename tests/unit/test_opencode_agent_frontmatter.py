"""I1 — Agent file (`.opencode/agent/corpus-forge-researcher.md`).

The agent file is what OpenCode reads when it wants to spawn a focused
"research librarian" against the corpus. Tests pin:

1. **Frontmatter** parses as YAML, declares ``name``, ``description``,
   ``model``, and a ``tools`` list with the five
...
AGENT_PATH = REPO_ROOT / ".opencode" / "agent" / "corpus-forge-researcher.md"
...
def test_agent_file_exists() -> None:
    """Agent lives at the well-known `.opencode/agent/<slug>.md` path."""
    assert AGENT_PATH.is_file(), f"missing {AGENT_PATH}"


def test_agent_frontmatter_parses_as_yaml() -> None:
    """Frontmatter must be valid YAML so OpenCode's agent loader accepts it."""
    fm, _ = _read_agent()
    assert isinstance(fm, dict)


def test_agent_frontmatter_has_required_keys() -> None:
    """`name`, `description`, `model`, `tools` are required by the agent schema."""
    fm, _ = _read_agent()
...
def test_agent_name_matches_filename_slug() -> None:
    """`name:` must equal the filename slug to avoid silent agent-name drift."""
    fm, _ = _read_agent()
...
def test_agent_model_is_bare_identifier() -> None:
    """Match existing agent style — bare model identifier, not quoted."""
    fm, _ = _read_agent()
    model = fm["model"]
...
def test_agent_tools_lists_five_mcp_corpus_forge_tools() -> None:
    """`tools` must enumerate all five mcp__corpus-forge__ read tools as a YAML list."""
    fm, _ = _read_agent()
    tools = fm["tools"]
