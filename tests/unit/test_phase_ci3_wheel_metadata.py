...
class TestWheelIdentity:
    def test_wheel_filename(self, built_wheel: Path) -> None:
        ...
        assert built_wheel.name.startswith("corpus_forge-0.1.0b8"), (
            f"Expected wheel name to start with corpus_forge-0.1.0b8; got {built_wheel.name}"
        )
    ...
    def test_metadata_version(self, metadata: dict) -> None:
        assert metadata["Version"] == "0.1.0b8"
