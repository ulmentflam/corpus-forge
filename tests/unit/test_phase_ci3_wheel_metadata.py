...
class TestWheelIdentity:
    def test_wheel_filename(self, built_wheel: Path) -> None:
        ...
        assert built_wheel.name.startswith("corpus_forge-0.1.0b1"), (
            f"Expected wheel name to start with corpus_forge-0.1.0b1; got {built_wheel.name}"
        )
...
class TestAuthor:
    def test_author_email(self, metadata: dict) -> None:
        # Hatchling may emit either Author-email: 'Evan Owen <evan@qwerky.ai>'
        # or split Author / Author-email fields. Accept either.
        ae = metadata["Author-email"] or ""
        a = metadata["Author"] or ""
        combined = f"{ae} {a}"
        assert "evan@qwerky.ai" in combined, (
            f"Expected evan@qwerky.ai in Author/Author-email; got {combined!r}"
        )
