"""I1 — Walkthrough doc (`docs/opencode-integration.md`).

The walkthrough is the human-readable handhold for getting corpus-forge
wired into OpenCode and asking the first question.
...
DOC_PATH = REPO_ROOT / "docs" / "opencode-integration.md"
...
def test_doc_has_all_required_h2_sections() -> None:
    ...
    for heading in _REQUIRED_H2:
        ...
        assert pattern.search(body), f"docs/opencode-integration.md missing '## {heading}'"
...
def test_doc_references_command_and_subagent_paths() -> None:
    """Subagent + First-search sections must point at the .opencode/ artefacts."""
    body = DOC_PATH.read_text(encoding="utf-8")
    assert ".opencode/command/corpus-forge-search" in body, "doc must reference the command path"
    assert ".opencode/agent/corpus-forge-researcher" in body, "doc must reference the agent path"
