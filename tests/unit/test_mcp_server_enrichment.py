...
def _call_tool_raw(server: Any, name: str, arguments: dict) -> object:
...
def _build_server(backend: SQLiteBackend, writes_enabled: bool = True) -> Server[Any]:
    retriever = _LexicalRetriever(backend)
    return build_server(retriever_builder=lambda: retriever, writes_enabled=writes_enabled)


def _search_first_hit(server: Server[Any], query: str, **extra: object) -> dict:
...
@pytest.fixture
def server_writes(backend: SQLiteBackend, seeded: dict[str, int]) -> Server[Any]:
    """Server with writes_enabled=True + seeded data."""
    return _build_server(backend, writes_enabled=True)


@pytest.fixture
def server_readonly(backend: SQLiteBackend, seeded: dict[str, int]) -> Server[Any]:
...
class TestAddLabelViaMCP:
    def test_happy_path_returns_label_id_and_created(
        self, backend: SQLiteBackend, seeded: dict, server_writes: Server[Any]
    ) -> None:
