...
class TestLatestUnfinishedIngestRunHostScope:
    ...
    Mirrors ``TestLatestUnfinishedIngestRunHostScope`` from
    ``test_postgres_ingest_runs.py`` for behavioral parity.

    RED condition: ``latest_unfinished_ingest_run`` currently accepts no
    parameters beyond ``self``.  Every call with ``host=...`` raises
    ``TypeError: latest_unfinished_ingest_run() got an unexpected keyword
    argument 'host'``.
    """

    def test_latest_unfinished_ingest_run_host_none_returns_any(self, tmp_path):
        """host=None (default) returns the most-recent unfinished row regardless of host.

        Two unfinished rows on different hosts.  Calling with no host argument
        (back-compat) must return the most-recent of either.
        """
        backend = _make_backend(tmp_path / "db.sqlite")

        run_a = _run_id("hs-none-a")
        backend.start_ingest_run(
            run_id=run_a,
            host="machine-a",
            pid=101,
        ...
        backend.finish_ingest_run(run_a, status="interrupted")

        time.sleep(0.015)

        run_b = _run_id("hs-none-b")
        backend.start_ingest_run(
            run_id=run_b,
            host="machine-b",
            pid=202,
        ...
        backend.finish_ingest_run(run_b, status="interrupted")
        ...
        # Also verify explicit host=None behaves identically to no-arg.
    ...
    def test_latest_unfinished_ingest_run_host_filter(self, tmp_path):
        """host='machine-a' returns machine-a's row; host='machine-b' returns machine-b's.

        Two unfinished rows exist — one per host.  The host filter must be
        exact: each filtered call returns only that host's row.
        """
        backend = _make_backend(tmp_path / "db.sqlite")

        run_a = _run_id("hs-filter-a")
        backend.start_ingest_run(
            run_id=run_a,
            host="machine-a",
            pid=11,
    ...
    def test_latest_unfinished_ingest_run_host_no_match(self, tmp_path):
    ...
    def test_latest_unfinished_ingest_run_host_ignores_completed_on_same_host(self, tmp_path):
        ...
        run_a_completed = _run_id("hs-status-a")
        backend.start_ingest_run(
        ...
        backend.finish_ingest_run(run_a_completed, status="completed")

        run_b_unfinished = _run_id("hs-status-b")
        backend.start_ingest_run(
        ...
        backend.finish_ingest_run(run_b_unfinished, status="interrupted")

        # machine-a only has a completed row — host + status filters must both
        # apply, yielding None.
    ...
    def test_latest_unfinished_ingest_run_back_compat_existing_callers(self, tmp_path):
        """Existing call-sites that pass no host argument continue to work.

        The no-host-arg (back-compat) path must still return the unfinished row.
        """
        backend = _make_backend(tmp_path / "db.sqlite")

        run_id = _run_id("hs-compat")
        backend.start_ingest_run(
            run_id=run_id,
            host="some-host",
            pid=999,
