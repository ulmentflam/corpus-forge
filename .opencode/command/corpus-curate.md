...
- The user has asked twice for the same fact and the cited chunks are
  ...
  another search.
- The user explicitly asks the librarian / corpus-curate command to run
  a curation pass.
...
- The user asks a question that needs a citation — that's a job for the
  `corpus-forge-search` command, not this one.
- The MCP server has `writes_enabled=False` — `commit_curation` will
  not be available and the loop is read-only. Surface that to the user
