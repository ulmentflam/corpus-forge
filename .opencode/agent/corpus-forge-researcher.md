...
4. For conversation datasets, use `mcp__corpus-forge__render_conversation`
   or `mcp__corpus-forge__list_chat_templates` when the question is about
   dialogue structure or chat formatting.
5. Compose a 1–3 sentence answer per claim, each followed by a citation
   in the form:
...
6. If `hits` is empty or no hit is on-topic, say so explicitly: "Not found
