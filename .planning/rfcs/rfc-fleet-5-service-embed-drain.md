...
- [x] Daemon embed-drain loop: per-lane fleet-2 claim → embed →
      ...
      telemetry per batch. — REBUILT (this PR) after the 2026-06-13
      `git checkout` incident destroyed the prior-session WIP. TDD from the
      surviving blueprint `tests/unit/test_embed_drain_loop.py` (17 tests):
      `daemon.py` `run_embed_drain_loop` / `_drain_lane_batch` /
      `_build_drain_lanes` / `_DrainLane`, doubling backoff
      `drain_idle_min`→`drain_idle_max`, release-in-finally, non-Postgres
      no-op. See incident: `proposed/approvals/incident-git-checkout-destroyed-fleet5-wip.md`.
- [x] `[service] embed_drain` / `ingest_watch` config (+ defaults:
      off/on locally, on/on for a joined host); daemon lifecycle wires
      drain loop and makes the ingest watcher optional. — REBUILT (this PR):
      `ServiceConfig` (`config.py`, `embed_drain=False`/`ingest_watch=True`,
      `extra='forbid'`) + `EmbedConfig.drain_idle_min`/`drain_idle_max` +
      `run_daemon` wiring (drain thread gated on `embed_drain`; ingest
      watcher gated on `ingest_watch`; shutdown joins the drain thread).
      14 tests in `test_service_config.py`. PR #148's join-seeding now has a
      `ServiceConfig` to consume.
- [x] `setup --join` seeds `embed_drain=true`; GPU-probe offers
      ...
      (PR #148): `setup` `--embed-drain/--no-embed-drain` +
      `--ingest-watch/--no-ingest-watch` (env `CF_EMBED_DRAIN` /
      `CF_INGEST_WATCH`); `run_join` resolves (embed_drain default-on for
...
- [x] Idle test: empty backlog → loop backs off, does not hot-spin
      ...
      min interval. — DONE (this PR): `tests/unit/test_embed_drain_loop.py`
      `test_backoff_capped_at_max` / `test_does_not_hot_spin` /
      `test_work_resets_backoff_to_min` / `test_empty_backlog_backs_off_exponentially`
      all green against the rebuilt loop.
- [x] CLAUDE.md "Add a second machine" + troubleshooting updates. — DONE
      (PR #148): `[service]` knobs + `--embed-drain`/`--ingest-watch` flags
      + pure-drain GPU recipe + backcompat note + a "backlog isn't
      draining" troubleshooting row.
