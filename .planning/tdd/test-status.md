...
|---------|--------|-------|
| chunker-hard-max-cap | red | 38 fail / 4 pass. 4 passes are vacuously correct: zero/negative/large-negative ValidationError tests pass because `extra="forbid"` rejects `chunker_hard_max_chars` before gt=0 fires; they remain green after the field lands. All 38 failures are correct-reason RED: config tests → AssertionError (field missing) or wrong-reason ValidationError; helper tests → ImportError (`enforce_chunk_hard_max` not importable from `corpus_forge.chunkers` or `corpus_forge.chunkers.hard_max`). Handed to tdd-coder. |
