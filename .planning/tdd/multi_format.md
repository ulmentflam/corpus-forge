...
**Phase D** (this file): **COMPLETE** (P0 closed at Wave 3, P1 closed at Wave 6). P0 gate closed — fixture corpus + e2e integration test + docs + pyproject extras; D-17..D-20 done. Wave 4 VLM foundation E-01..E-04 done — committed at `acdfa83`. Wave 5 PDF/image OCR integration E-05..E-06 done — committed at `0f0d102`. Wave 6 P1 gate — E-07..E-10 done — P1 fixtures (`pdf/scanned-paper.pdf` + `images/{screenshot.png, photo-of-receipt.jpg, diagram.webp}`) generated by `scripts/build_fixture_corpus.py::build_p1_ocr_fixtures` (deterministic across runs); live-Ollama e2e (`test_ocr_local_e2e.py`, 4 tests, ~30 s on M-series with qwen2.5vl:7b); Mistral e2e (`test_ocr_remote_e2e.py`, 3 tests, auto-skip on `MISTRAL_API_KEY` absence); `pyproject.toml` markers `requires_ollama` + `requires_mistral_api` registered; collection-time skip plumbing in `tests/integration/conftest.py`; `make test-ocr` / `make test-ocr-local` targets; `secrets.env.example` carries `MISTRAL_API_KEY=`; `docs/architecture.md` Vision/OCR section + README `[ocr]` extra note. **P1 gate closed at <commit-hash-tbd>** (orchestrator fills hash after signed commit).
...
| E-07 | OCR local E2E test | E-05 | `tests/integration/test_ocr_local_e2e.py`, fixture scanned-paper PDF + `images/*` | med | done | principal | 4 live-Ollama tests (PDF escalation happy path, ImageExtractor happy path, all-three-image-fixtures round-trip, VLMTimeoutError degradation). Gated by `requires_ollama` marker; conftest auto-skips when daemon down or model missing. Per-test `@pytest.mark.timeout(120-240)` headroom for first-call model warm-up. Fixtures generated by `build_p1_ocr_fixtures()` in `scripts/build_fixture_corpus.py`; PIL bundled DejaVu Sans Bold + pinned encoder options → byte-deterministic across runs. The scanned-paper PDF has zero text-layer chars → `PdfDigitalExtractor` flags `sparse_text_layer=True` and escalates. Live run: 4/4 passed in ~27 s on M-series. P0 e2e test (`test_multi_format_ingest_e2e.py`) was updated to add the 3 image fixtures to `_UNINGESTABLE` (they're correctly skipped under the no-VLM default `ExtractionConfig`). |
| E-08 | OCR remote E2E test | E-05 | `tests/integration/test_ocr_remote_e2e.py` | med | done | principal | 3 Mistral-API tests (PDF escalation, ImageExtractor, all-three-image-fixtures round-trip); 401 case dropped per the wave spec (Wave 4 unit tests cover the exception map with mocked HTTP). `requires_mistral_api` marker; conftest skips the whole file when `MISTRAL_API_KEY` is unset (the current state on this machine). Live API roundtrip ready-to-run when the key is wired in. |
| E-09 | Makefile + secrets.env + docs | E-07, E-08 | `Makefile`, `secrets.env.example`, `docs/architecture.md`, `README.md`, `pyproject.toml` | low | done | principal | New `make test-ocr` (both markers) and `make test-ocr-local` (Ollama only) Makefile targets. `secrets.env.example` gains `MISTRAL_API_KEY=` with one-line comment pointing at Mistral OCR docs. `pyproject.toml` `[tool.pytest.ini_options].markers` extended with `requires_ollama` + `requires_mistral_api`. `docs/architecture.md` "Vision/OCR pipeline" H3 added — VLM protocol, backend matrix (Ollama/Mistral/Noop), tier 1→tier 2 escalation, ImageExtractor, failure ladder, marker convention. `README.md` Distribution/licensing section extended with a paragraph clarifying that `[ocr]` is light (no model weights vendored; backends speak HTTP) and does not widen the AGPL surface. |
| E-10 | **P1 gate** — `make ci` green + live-Ollama smoke | E-09 | — | gate | done | principal | `make ci` exit 0: 2696 unit pass + 2 skipped + 1 xfailed @ 92.35% coverage (≥90% gate); 382 integration pass + 3 Mistral skips; 15 fuzz pass; 30 smoke pass. `make test-ocr-local`: 4 live-Ollama tests pass in 38 s (qwen2.5vl:7b). `make test-ocr` (full OCR suite) skips Mistral cleanly when `MISTRAL_API_KEY` is unset. Determinism contract verified — `python scripts/build_fixture_corpus.py` run twice → byte-identical fixtures. **Hard-rule honoured: no commits made by principal during Wave 6** (per user directive after the Wave 5 unsigned-commit incident). |
