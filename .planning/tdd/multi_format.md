...
**Phase D** (this file): **P0 complete; P1 Wave 5 done** (P0 gate closed at Wave 3 — fixture corpus + e2e integration test + docs + pyproject extras; D-17..D-20 done. Wave 4 VLM foundation E-01..E-04 done — protocol + Ollama + Mistral backends + VLMConfig — committed at `acdfa83`. Wave 5 PDF/image OCR integration E-05..E-06 done — PDF Tier 2 escalation + ImageExtractor + registry wiring — pending principal commit; Wave 6 P1 gate next).
...
| E-05 | `PdfExtractor` escalation upgrade | E-02, E-03, E-04 | `corpus_forge/extractors/pdf.py`, `tests/unit/test_pdf_extractor_escalation.py` (+ cross-cutting: `config.py`, `extractors/registry.py`, `ingest.py`, `sources/filesystem.py`, `pyproject.toml`, `README.md`) | high | done | principal (no Agent tool) | 17 tests. Tier 2 = pdf2image (lazy) + per-page `VLMBackend.extract_page`. NoopVLM short-circuits silently (Option 1). Failure handling: VLMUnavailable / VLMResponseError → graceful Tier 1 fallback; VLMTimeoutError → per-page `<!-- VLM timeout on page N -->` placeholder + continue; PDFInfoNotInstalledError → ERROR log + Tier 1. `_resolve_pdf2image()` module-private helper resolves test-time monkeypatches without self-import. ExtractionConfig knobs added: `ocr_enabled` / `ocr_min_chars_per_page` / `ocr_dpi` / `enable_image`. Registry signature widened: `register_default_extractors(config, vlm=None)`. ingest._instantiate_source builds VLM via `get_active_vlm(config)` and threads through FilesystemSource. rag-helper import path preserved (regression-pinned). |
| E-06 | `ImageExtractor` | E-02, E-03, E-04 | `corpus_forge/extractors/image.py`, `tests/unit/test_extractor_image.py` | med | done | principal (no Agent tool) | 17 tests. Thin shim over `VLMBackend.describe_image`. `vlm` constructor-injected (keyword-only). Default prompt biases verbatim transcription. Supports `.png .jpg .jpeg .tif .tiff .bmp .webp .heic` (HEIC decode is the VLM's job — docstring points to pillow-heif for users whose VLM can't). Multi-page TIFF out of scope. Registry registration handled by E-05's upgraded `register_default_extractors` (gated on `vlm is not None` + non-NoopVLM + `ocr_enabled` + `enable_image`). 100% file coverage. |
