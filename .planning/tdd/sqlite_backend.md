...
| B-03 | `SQLiteBackend` skeleton + `migrate()` | B-01, B-02 | `corpus_forge/backends/sqlite.py`, `tests/unit/test_sqlite_backend.py` | med | in_progress | tdd-coder | 28/29 tests green. One conflict: `test_no_postgres_backfill_sql_executed` (line 401-424) checks that no SQL passed to `_execute` contains "SHA256". `001_core.sql` line 35 has inline comment `-- sha256 of raw bytes (idempotency key)` in the documents CREATE TABLE DDL. `apply_migrations` passes the raw statement (with comment) to `_execute`; spy sees "sha256" in the comment. Fix: tdd-tester must either (a) narrow the check to UPDATE-only statements, or (b) strip inline comments from `sql_upper` before checking, or (c) tdd-B-02 removes the `sha256` inline comment from `001_core.sql` line 35. Cannot fix without touching the test or frozen B-02 schema file. |
