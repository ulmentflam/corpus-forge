...
| B-05 | `upsert_document` + chunk reuse | B-04 | `corpus_forge/backends/sqlite.py`, `tests/unit/test_sqlite_backend.py` | high | red | tdd-tester | 32 tests written across 6 test classes (TestUpsertDocumentNewDocument, TestUpsertDocumentExistingDocument, TestUpsertDocumentChunkReuse, TestUpsertDocumentEmbedderIds, TestCopyReusableEmbeddings, TestUpsertDocumentSqliteDialect, TestUpsertDocumentState, TestUpsertDocumentFailurePaths). All fail with AttributeError (method not yet implemented). 1 green: test_unique_constraint_on_documents_exists. **Vocab fix by principal**: row was incorrectly marked `done`; corrected to `red`. **Gate hygiene fix**: 16 E501 line-too-long errors in the test file were wrapped by principal in a follow-up commit so coder's gates stay clean. |
