...
| B-07 | `write_embeddings` + `chunks_missing_embedding` | B-04 | `corpus_forge/backends/sqlite.py`, `tests/unit/test_sqlite_backend.py` | med | red | tdd-tester | **17 tests red** (8 TestWriteEmbeddings + 9 TestChunksMissingEmbedding). sqlite-vec accepts numpy arrays as serialized FLOAT32 blobs. Use `sqlite_vec.serialize_float32(np_array)`. Fallback (no sqlite-vec): `np.tobytes()` straight to BLOB column. `chunks_missing_embedding(embedder_id)` returns chunk rows lacking a row in `embeddings_<name>` for that `embedder_id`. |
