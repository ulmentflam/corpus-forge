...
| B-18 | E2E smoke: ingest a markdown vault into SQLite end-to-end | B-13, B-15 | `tests/smoke/test_smoke_sqlite.py` | low | in_progress | tdd-tester | One-test smoke using `tmp_path` for both vault and db. Confirms wiring works from CLI/daemon entry through to a queryable SQLite file. Bug surfaced: `ingest.py:_get_or_create_dataset` uses `corpus.` schema prefix + `%s` Postgres placeholders — SQLite rejects both. Coder must fix. Fake embedder: `_FakeSmokeEmbedder` (sha256-derived 8-dim, no model load). Chunker: MarkdownChunker(max_chars=500, overlap=50), 1 chunk/file. Expected: 3 docs, 3 chunks, 1 embedder. |
