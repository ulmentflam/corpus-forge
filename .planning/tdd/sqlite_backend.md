...
| B-06 | `upsert_conversation` | B-04 | `corpus_forge/backends/sqlite.py`, `tests/unit/test_sqlite_backend.py` | med | claimed | tdd-tester | **Claimed by session `946880f9` (this principal)**. Parallel agent owns B-05 lane on the same `sqlite.py` file. Coordination plan: B-06 tester writes additive `TestUpsertConversation` class to `tests/unit/test_sqlite_backend.py`; B-06 coder waits for parallel B-05 work to commit before editing `sqlite.py` to avoid merge conflicts on the same file. Mirror `postgres.py:upsert_conversation`. Conversations + messages + per-message chunks. |
