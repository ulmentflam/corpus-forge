...
| B-06 | `upsert_conversation` | B-04 | `corpus_forge/backends/sqlite.py`, `tests/unit/test_sqlite_backend.py` | med | red | tdd-tester | 21 tests written (5 new + 4 existing + 6 messages + 4 chunks + 2 failure). All red: `AttributeError: 'SQLiteBackend' object has no attribute 'upsert_conversation'`. Pre-existing 70 tests unaffected. Mirror `postgres.py:upsert_conversation`. Conversations + messages + per-message chunks. |
