...
| B-15 | Integration tests for SQLite backend (mirror PG suite) | B-05..B-12 | `tests/integration/test_backend_sqlite.py`, `tests/integration/test_migrate_sqlite.py` | med | qa-approved | tdd-qa | 107 passed (63 in test_backend_sqlite.py + 44 in test_migrate_sqlite.py), 0 failed, 0 skipped (sqlite-vec available so vec0 test ran). PRAGMA patterns: sqlite_master table enum, PRAGMA table_info, PRAGMA foreign_key_list, PRAGMA foreign_keys. vec0-gated subset: 1 test gated on SQLITE_VEC_AVAILABLE. No `pytest.mark.integration` pytestmark on either file. Characterization tests pin existing SQLiteBackend behavior. All 4 gates clean: format, lint, pyrefly n/a (tests only). |
| B-16 | Parametrize a subset of tests across both backends | B-15 | `tests/conftest.py`, `tests/integration/test_backend_dual.py` | med | qa-approved | tdd-qa | 17 parametrized tests x 2 backends = 34 executions all green. `backend_kind` fixture params `["postgres","sqlite"]`; `storage_backend` fixture yields migrated backend instance, skips postgres if Docker unavailable. No `pytestmark=pytest.mark.integration` (SQLite must work without Docker; postgres skip handled in fixture via `pytest.skip`). TestUpsertDocumentSmoke (8), TestChunkReuseE2E (2), TestRevisions (7). All 4 gates clean: format, lint, pyrefly 0 errors, 34/34 dual-backend + 243/243 full integration + 1067/1067 unit. |
| B-17 | Update `docs/architecture.md`, `docs/schema.md`, `README.md` | B-13 | `docs/architecture.md`, `docs/schema.md`, `README.md`, `config.example.toml` | low | green | doc-writer | LOC added (per `git diff --stat`): docs/architecture.md +26, docs/schema.md +26, README.md +38 net (-6 stale lines flipped), config.example.toml +10. Net 100 insertions / 8 deletions. Surprises while grounding: (1) Postgres SQL lives at top-level `corpus_forge/schema/`, not `schema/postgres/` as the board spec implied — the `dialect="postgres"` branch in `migrate.py` reads from `schema_dir` directly. Docs reflect actual layout. (2) `_execute` strips `AUTOINCREMENT` at run time even though the SQLite DDL files contain it; called out in the dialect table. (3) `lock_source` `key` arg is accepted but ignored on SQLite (uses the global write lock); called out in both the architecture table and the schema dialect table. (4) Existing README "Configuration reference > [backend]" said `kind: "postgres" (planned: "sqlite")` and "Supported backends" listed SQLite as planned — both flipped to reflect shipped reality. No other stale docs spotted. |
| B-18 | E2E smoke: ingest a markdown vault into SQLite end-to-end | B-13, B-15 | `tests/smoke/test_smoke_sqlite.py` | low | qa-approved | tdd-qa | One-test smoke using `tmp_path` for both vault and db. Confirms wiring works from CLI/daemon entry through to a queryable SQLite file. Bug surfaced: `ingest.py:_get_or_create_dataset` uses `corpus.` schema prefix + `%s` Postgres placeholders — SQLite rejects both. Fixed by lifting get_or_create_dataset / find_dataset_id_by_name / register_source into StorageBackend protocol. Smoke: 1/1 green; unit: 1067/1067; integration: 243/243. |
