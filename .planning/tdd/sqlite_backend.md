...
| B-14 | Daemon rejects `sync_enabled = true` for SQLite | B-13 | `corpus_forge/daemon.py`, `corpus_forge/config.py` (validator), `tests/unit/test_daemon.py`, `tests/unit/test_config.py` | low | green | tdd-coder | 86 passed, 1 xfailed (scoped run). `validate_sync_gate` added to `Config` — 10 previously-red tests now green, 0 regressions. |
