...
|----|-------|------------|---------|------|--------|------------|-------|
| P0-01 | `chunk_content_hash` helper in `identity.py` | — | `corpus_forge/identity.py`, `tests/unit/test_identity.py` | low | in_progress | tdd-tester | Trivial: thin wrapper around `content_hash(text.encode())`. |
...
| P1-01 | `003_sync.sql` migration file (DDL only) | P0-02 (file ordering) | `corpus_forge/schema/003_sync.sql` | low | in_progress | tdd-tester | `document_revisions`, `documents.tombstoned_at`, `sources.last_pulled_revision_id`, `sources.sync_enabled`. |
...
| P1-03 | Pydantic config: `DaemonConfig` sync fields + `DatasetConfig.sync_enabled` + validators | — | `corpus_forge/config.py`, `tests/unit/test_config.py` (and/or test_config_extended) | low | in_progress | tdd-tester | `host_id`, `trash_dir`, `conflict_dir`, `sync_poll_interval_s`, `sync_use_listen_notify`. Validator: `sync_enabled` only when `kind == "text"`. |
...
| P1-05 | `config.example.toml` + default exclude_globs include `*.icloud` | P1-03 | `config.example.toml` | low | in_progress | tdd-tester | Documentation-shaped task; no test, but ruff/format must still pass. |
| P1-06 | `EchoSuppressor` in `sync/echo.py` | — | `corpus_forge/sync/__init__.py`, `corpus_forge/sync/echo.py`, `tests/unit/test_sync_echo.py` | low | in_progress | tdd-tester | API: `register(path, content_hash, ttl=5.0)`, `was_just_written(path, content_hash) -> bool`, `gc()`. Thread-safety choice → see open-questions.md. |
| P1-07 | `sync/cloud.py::detect_cloud_provider` | — | `corpus_forge/sync/cloud.py`, `tests/unit/test_sync_cloud.py` | low | in_progress | tdd-tester | Substring/prefix match on resolved abs path. Returns `Literal["icloud","dropbox","gdrive","none"]`. |
...
| P1-09 | `sync/conflicts.py::conflict_filename` (canonical conflict naming) | — | `corpus_forge/sync/conflicts.py`, `tests/unit/test_sync_conflicts.py` | low | in_progress | tdd-tester | `<stem>.conflict-<host>-<ts><suffix>`; provider-tagged variant `<stem>.conflict-<provider>-<host>-<ts><suffix>`. |
| P1-10 | `sync/fs.py::atomic_write_text` | — | `corpus_forge/sync/fs.py`, `tests/unit/test_sync_fs.py` | low | in_progress | tdd-tester | Tempfile-in-same-dir + `os.replace`. UTF-8, preserves no extra metadata. |
