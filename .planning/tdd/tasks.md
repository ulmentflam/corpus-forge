...
| id | title | depends_on | surface | risk | status | claimed_by | notes |
|----|-------|------------|---------|------|--------|------------|-------|
| R3-01 | Add `[retrieval]` + `[eval]` extras to pyproject | — | `pyproject.toml`, `tests/unit/test_pyproject_eval_extras.py` | low | red | tdd-tester | 7 tests RED (3 extras-present + 1 floor + 1 importable + 2 scope-guards already green). |
| R3-02 | `eval/metrics.py` (ndcg_at_k, mrr_at_k, recall_at_k) | — | `corpus_forge/eval/metrics.py`, `tests/unit/test_eval_metrics.py` | low | red | tdd-tester | 29 tests RED. Hand-computed known answers (binary + graded NDCG); MRR rank-1/rank-5/no-hit/outside-k; recall edge cases; k>len tolerated. |
| R3-03 | `eval/dataset.py` — JSONL loader + `GoldQuery` | — | `corpus_forge/eval/dataset.py`, `tests/unit/test_eval_dataset.py` | low | red | tdd-tester | 21 tests RED. GoldQuery shape, happy + error paths, str→int graded normalisation, content_hashes length check, line-numbered error messages. |
