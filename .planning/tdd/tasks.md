...
| R3-04 | `eval/runner.py` — `evaluate_retriever` + `report` + JSON dump + pinned baseline test | R3-02, R3-03 | `corpus_forge/eval/runner.py`, `tests/unit/test_eval_runner.py` | med | red | tdd-tester | 11 tests RED. Pinned NDCG@10 floor 0.80 on SQLite-backed toy corpus + FakeEmbedder; break-the-retriever sanity test (constant query vec + alpha=1.0) MUST drop below floor. |
