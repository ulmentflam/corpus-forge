...
| D-09 | Smoke: `corpus-forge mcp serve` boots clean against Alembic'd DB | D-07 | `tests/smoke/test_mcp_serve_boots_with_alembic.py` | med | done | tdd-tester | Wave 3; RED `<sha>`; 3/6 fail with `ModuleNotFoundError: No module named 'mcp'` — coder must install [mcp] extra; stdout MUST stay JSON-RPC-clean (no migration noise leaks past stderr) |
