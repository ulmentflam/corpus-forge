...
| D-08 | CLI subcommands `migrate revision` + `migrate history` | D-07 | `corpus_forge/cli.py`, `tests/unit/test_cli_migrate.py` | low | done | tdd-tester | Wave 3; RED `78e537a`; 4/4 tests fail "Got unexpected extra argument" — coder lands subcommands |
| D-09 | Smoke: `corpus-forge mcp serve` boots clean against Alembic'd DB | D-07 | `tests/smoke/test_mcp_serve_boots_with_alembic.py` | med | done | tdd-tester | Wave 3; RED `9e95934`; 3/6 fail with `ModuleNotFoundError: No module named 'mcp'` — coder must install [mcp] extra; stdout MUST stay JSON-RPC-clean (no migration noise leaks past stderr) |
