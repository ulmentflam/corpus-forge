...
| P0-08 | E2E reuse test (testcontainers) | P0-03..P0-07, INT-01 | `tests/integration/test_chunk_reuse_e2e.py` | med | red | tdd-tester | Wave 13b: test written, 2 failures surface real production bugs: (1) `_copy_reusable_embeddings` INSERT missing `embedder_id NOT NULL`; (2) `upsert_document` deletes chunks before reuse lookup — self-defeating. Bugs reported below; principal must spawn coder task. |
...
| P1-30 | E2E push→pull integration test (testcontainers) | P1-27, P1-28, INT-01 | `tests/integration/test_sync_push_pull.py` | high | red | tdd-tester | Wave 13b: 4 tests written, all red due to real production bugs. BUG-1: `push.py:84` calls `backend.resolve_document()` (missing from PostgresBackend). BUG-2: `insert_revision` call in push.py missing `source_uri` kwarg. BUG-3: `pending_remote_revisions` SELECT r.* lacks `source_uri`/`source_id` from documents/sources. BUG-4: pull.py:69 absolute-path drop (`Path(root_b) / "/absolute"` → `/absolute`). Needs INT-03 coder task to fix production code. |
| P1-31 | E2E tombstone integration test | P1-25, P1-27, INT-01 | `tests/integration/test_sync_tombstone.py` | med | red | tdd-tester | Wave 13b: 13 tests written, all red. Same root bug as P1-30: `push.py:84` calls `backend.resolve_document()` which does not exist on PostgresBackend (AttributeError). Latent bugs: BUG-PULL-SOURCE-URI (pending_remote_revisions returns r.* with no source_uri column; pull.py:69 also drops source_root on absolute path); BUG-PULL-SOURCE-ID (mark_revision_pulled needs source_id not in document_revisions). Needs INT-03 coder fix. Tests must NOT be relaxed. |
| P1-32 | E2E iCloud-duplicate integration test | P1-08, P1-20, INT-01 | `tests/integration/test_sync_icloud_dupe.py` | med | done | tdd-tester | Wave 13: 5 tests written (2 pass green, 3 red). iCloud substring detection confirmed working under tmp_path. 3 failures surface two production bugs: BUG-PUSH-DUPE (handle_change never calls _handle_cloud_duplicate; watchdog routing is broken) and BUG-PUSH-RESOLVE (handle_change + _handle_cloud_duplicate call backend.resolve_document() which does not exist on PostgresBackend). Tests must NOT be relaxed — failures are real production bugs. Needs tdd-coder to fix both bugs before green. |
