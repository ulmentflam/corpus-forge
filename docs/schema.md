...
| `0016_chunk_provenance` | Adds five nullable provenance columns to `chunks`: `file_path TEXT`, `line_start INTEGER`, `line_end INTEGER`, `git_commit TEXT`, `git_branch TEXT`. Existing rows survive untouched with NULL provenance; subsequent ingests re-emit the values once the chunker + source wiring tasks of RFC `rfc-source-provenance-git-and-lines` land. Idempotent on both backends — Postgres uses `ADD COLUMN IF NOT EXISTS`, SQLite uses a `PRAGMA table_info` probe. Forward-only per the chain (0008+). |
| `0017_ingest_runs` | Adds `ingest_runs` (one row per ingest pass: `run_id`, `status`, progress counters, `host`, `pid`, `config_digest`, timestamps) and `ingest_run_sources` (one row per `(run_id, source_uri_prefix)`: per-source scan freshness and per-document counters). Enables stop-and-resume via `corpus-forge ingest --once --resume`. Both tables use `CREATE TABLE IF NOT EXISTS` — fully idempotent on both backends. Also adds `DEFAULT 'text'` to `datasets.kind` on SQLite via table-recreation (same pattern as 0009). Forward-only per project convention (0008+). |
| `0018_model_telemetry` | Adds the fleet telemetry registry of RFC `rfc-fleet-1-model-telemetry-and-bench`: `hosts` (one row per machine, keyed by `host_id`; `hostname`, `os`, `accelerator` JSONB probe output, optional `tailscale_name`, `last_seen` heartbeat), `models` (registry keyed by `"<provider>:<model_id>"`; `kind`, `provider`, `model_id`, nullable `dimension`, `first_seen`), and `model_benchmarks` (one row per measured throughput sample, FK to `hosts(host_id)` + `models(model_key)`; `source`, `transport`, `device`, `batch_size`, `sample_chunks`, `chunks_per_s`, `tokens_per_s`, latency percentiles, `measured_at`) plus the `model_benchmarks(host_id, model_key, measured_at DESC)` "latest per host+model" index. All three use `CREATE TABLE IF NOT EXISTS` — fully idempotent on both backends. SQLite stores `accelerator` as JSON `TEXT` and timestamps as ISO-8601 `TEXT` (same dialect split as 0017). Forward-only per project convention (0008+). |
| `0019_embed_claims` | Adds `embed_claims`, the distributed claim primitive of RFC `rfc-fleet-2-distributed-embedding`: one row per `(embedder_id, chunk_id)` a host has reserved for embedding (`claim_id` PK, `host_id` FK → `hosts(host_id)`, `claimed_at`, `lease_until = claimed_at + lease_ttl`), with `UNIQUE (embedder_id, chunk_id)` and an `embed_claims(lease_until)` index for the lease-expiry sweep. Lets N hosts drain the same embedder lane concurrently with zero duplicated GPU compute (`FOR UPDATE SKIP LOCKED` + `ON CONFLICT DO NOTHING` in `PostgresBackend.claim_chunks_for_embedding`); abandoned claims self-heal via `expire_stale_claims`. Created on both dialects for schema symmetry (`CREATE TABLE IF NOT EXISTS`, idempotent), but the SQLite backend raises `FederationUnsupported` for all three claim methods (RFC non-goal: no SQLite federation). Unlike the forward-only 0008+ chain, `downgrade()` drops the table — it holds only ephemeral coordination state (the embedding rows are the durable record). |
| `0020_shared_config` | Adds `shared_config`, the storage half of RFC `rfc-fleet-3-federated-config-and-setup`: one row holding the corpus's canonical shared config (`corpus_id` PK `DEFAULT 1` — one corpus per DB, PK kept for forward room; `version` monotonic publish counter; `body` JSONB blob; `published_by` FK → `hosts(host_id)`; `published_at`). `config publish` bumps `version` under an optimistic-concurrency guard so a stale publisher can't blind-clobber a peer's newer config; `config pull` reads it back. Writes go through `PostgresBackend.put_shared_config`, a single atomic statement (`INSERT … ON CONFLICT (corpus_id) DO NOTHING RETURNING version` on first publish; `UPDATE … WHERE corpus_id = 1 AND version = %s RETURNING version` on updates) — a lost race returns no row and raises `SharedConfigVersionConflict` ("pull first"). Created on both dialects for schema symmetry (`CREATE TABLE IF NOT EXISTS`, idempotent; `body`/`published_at` are `TEXT` on SQLite), but the SQLite backend raises `FederationUnsupported` for both `get_shared_config`/`put_shared_config` (RFC non-goal: no SQLite federation). Like 0019, `downgrade()` drops the table — it holds only federated coordination state each host also keeps locally in `config.toml`. |
| `0021_benchmark_cold_start` | Adds the nullable `model_benchmarks.cold_start_s` column (the stretch task of RFC `rfc-bench-embed-progress`): the model load + warmup wall clock that `bench embed` already measures (PR #121) and shows in its Rich table + `--json`, now persisted so `models list` can compare cold-start cost across lanes the way it compares `chunks_per_s`. `NUMERIC` on Postgres, `REAL` on SQLite, nullable — old rows and the passive `embed-run` telemetry path (which never times a discrete cold start) keep `NULL`; only `bench` writes the float. Purely additive (one `ADD COLUMN`, no index touched), idempotent on both dialects (Postgres `ADD COLUMN IF NOT EXISTS`; SQLite, which has no `IF NOT EXISTS` for `ADD COLUMN`, introspects `PRAGMA table_info` first). Like 0019/0020, `downgrade()` drops the column — telemetry-only data, nothing durable lost on rollback. |
