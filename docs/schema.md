...
| `0021_benchmark_cold_start` | Adds the nullable `model_benchmarks.cold_start_s` column (the stretch task of RFC `rfc-bench-embed-progress`): the model load + warmup seconds `bench embed` already computed for its `--json` payload, now persisted so the fixed spin-up cost a warm run avoids is durable and surfaced by `models list`. Additive + nullable — old rows read `NULL`, no backfill. Idempotent on both dialects (Postgres `ADD COLUMN IF NOT EXISTS`; SQLite `PRAGMA table_info` probe before `ADD COLUMN`, since SQLite has no `IF NOT EXISTS` for columns); `downgrade()` drops the column where supported (`DROP COLUMN` landed in SQLite 3.35, else a logged no-op). |
