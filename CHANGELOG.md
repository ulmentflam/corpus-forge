...
- `PostgresBackend` now registers an ``atexit`` callback to close
  ...
  already" on ~6 alembic-migration tests. ``atexit`` fires only
  at interpreter shutdown so it can't disrupt test-time
  connection state. Regression coverage in the new
  ...
  idempotent, ``close()`` actually closes the pool, dead-weakref
  in the atexit callback is a no-op, and pool-close exceptions
