...
- **`setup --join` now turns on the embed-drain loop for the joined host**
  (RFC fleet-5). The joined host's rendered `config.toml` gets a
  ...
  stays off (no surprise background GPU loop on a laptop).
